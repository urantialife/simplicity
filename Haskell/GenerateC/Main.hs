...
data Example = Example { _name :: String
                       , _path :: [String]
                       , _text :: [String]
                       , _prog :: WrappedSimplicity () ()
                       }
name f (Example n pa t pr) = (\n -> Example n pa t pr) <$> f n
path f (Example n pa t pr) = (\pa -> Example n pa t pr) <$> f pa
text f (Example n pa t pr) = (\t -> Example n pa t pr) <$> f t
prog f (Example n pa t pr) = (\pr -> Example n pa t pr) <$> f pr
fullname :: Getter' Example String
fullname = to proj
 where
...
showComment txt = unlines $ ["/* A length-prefixed encoding of the following Simplicity program:"]
               ++ body
               ++ [" * with jets.", " */"]
 where
  body = map (" *     "++) $ txt
...
showHash name h = unlines $ ["const uint32_t "++name++"[] = {", body, "};"]
 where
  str_h = padding ++ text
   where
    padding = replicate (32 - length text) '0'
...
fileC example = "#include \""++example^.name++".h\"\n"
 ...
 where
  binary = BS.unpack . runPut . putBitStream . putTermLengthCode . unwrap $ example^.prog
